 ...
 *  3. 한 손가락 (접근성·한 손): 톡 세 번(1.4초 안) 또는 꾹 0.9초
...
/** 한 손가락 톡: 최근 톡 목록에 now 를 더하고 진행도를 돌려준다 */
export function registerCapsuleTap(taps: readonly number[], now: number): { taps: number[]; progress: number } {
  const recent = taps.filter((t) => now - t < CAPSULE_TUNING.tapWindowMs);
  recent.push(now);
  return { taps: recent, progress: Math.min(1, recent.length / CAPSULE_TUNING.tapsToOpen) };
}
